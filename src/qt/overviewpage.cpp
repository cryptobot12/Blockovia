...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::BCK)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("BCK")
    updateDisplayUnit();
}
...
void OverviewPage::updatePrivateSendProgress()
{
    ...
    QString strAnonymizeblockoviaAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeblockoviaAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeblockoviaAmount = strAnonymizeblockoviaAmount.remove(strAnonymizeblockoviaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeblockoviaAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeblockoviaAmount * COIN) nMaxToAnonymize = nAnonymizeblockoviaAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeblockoviaAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeblockoviaAmount));
        strAnonymizeblockoviaAmount = strAnonymizeblockoviaAmount.remove(strAnonymizeblockoviaAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeblockoviaAmount + " / " + tr("%n Rounds", "", nPrivateSendRounds);
    } else {
...
void OverviewPage::togglePrivateSend()
{
    ...
    if (!fEnablePrivateSend) {
        ...
        if (nAnonymizeblockoviaAmount == 0) {
