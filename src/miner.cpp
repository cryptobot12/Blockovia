...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("BCKMiner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("BCKMiner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("BCKMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("blockovia-miner");
...
///////|| !masternodeSync.IsSynced()
        ...
        LogPrintf("Running BCKMiner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
